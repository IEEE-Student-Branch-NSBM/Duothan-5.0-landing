...
const Header = ({ disableScaling = false }: HeaderProps = {}) => {
	...
	return (
		<div
			ref={headerRef}
			className={`fixed w-full ${isMobile ? "h-[100px]" : "h-[200px]"} overflow-hidden z-50 ${!disableScaling && scaleStyle ? "-top-3" : ""}`}
		...
		>
			<div className="relative w-full h-full flex flex-col items-center">
				...
				<div
					ref={layer3Ref}
					className={`absolute ${isMobile ? "hidden md:flex" : "flex"} top-20 left-0 w-full h-25 items-start justify-center brightness-125 flickering`}
				>
					<Image
				...
				<div
					ref={layer2Ref}
					className={`absolute ${isMobile ? "scale md:flex" : "flex"} top-0 left-0 w-full h-40 items-start justify-center [filter:drop-shadow(0_0_18px_rgba(149,76,233,0.6))]`}
				>
					<Image
				...
				{/* layer 1 - Only this will be visible on mobile */}
				<div
					ref={layer1Ref}
				...
				{/* logo - Always visible */}
				<div ref={logoRef} className={`${isMobile ? "mt-2" : "mb-4"}`}>
					<Image
						src="/assets/header/logo.svg"
						alt="Duothan Logo"
						width={320}
						height={120}
						className={`${isMobile ? "h-14" : "h-20"} w-full`}
					/>
				</div>
				...
				<div
					ref={buttonRef}
					className={`absolute inset-0 ${isMobile ? "top-8" : "top-12"} flex items-center justify-center text-white text-xs font-mono tracking-widest`}
				>
					<Image
						...
						width={246}
						height={23}
						className={`w-auto ${isMobile ? "h-5" : "h-6"}`}
					/>
