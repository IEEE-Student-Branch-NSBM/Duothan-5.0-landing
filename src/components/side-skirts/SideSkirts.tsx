...
const SideSkirts = ({
	...
	return (
		<div className="fixed inset-0 pointer-events-none z-10 hidden sm:flex justify-between items-stretch w-full h-full overflow-hidden">
			...
			{/* Right side skirts */}
			<div
				className={`relative h-full w-24 md:w-36 ${isVisible ? "slide-in-right" : "opacity-0"} ${!disableScaling && scaleStyle ? "right-0" : ""}`}
				style={disableScaling ? {} : scaleStyle}
			>
				...
				<div className="absolute right-5 left-0 mx-auto h-full w-10 flex justify-center items-center">
					...
					/>
					<div
						className="absolute flex flex-col items-center justify-center gap-5 transform scale-x-[-1] scale-y-[-1]"
						style={disableScaling ? {} : scaleStyle}
					>
